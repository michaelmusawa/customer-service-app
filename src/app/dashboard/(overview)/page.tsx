...
export default async function Page() {
  ...
  const [
    monthlyGroupedRecords,
    weeklyGroupedRecords,
    dailyGroupedRecords,
    yearlyGroupedRecords,
  ] = await Promise.all([
    fetchMonthlyGroupedRecords(),
    fetchWeeklyGroupedRecords(),
    fetchDailyGroupedRecords(),
  ...
  if (user?.role === "admin" || user?.role === "supersupervisor") {
    ...
    localYearlyGroupedRecords = yearlyGroupedRecords?.filter(
      (record) => record.userStation === user?.station
    );
    localMonthlyGroupedRecords = monthlyGroupedRecords?.filter(
      (record) => record.userStation === user?.station
    );
    localWeeklyGroupedRecords = weeklyGroupedRecords?.filter(
      (record) => record.userStation === user?.station
    );
    localDailyGroupedRecords = dailyGroupedRecords?.filter(
      (record) => record.userStation === user?.station
    );
  }
